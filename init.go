...
var proximaxInitCmd = &cmds.Command{
	Helptext: cmdkit.HelpText{
		Tagline: "Initializes ProximaX config file",
		ShortDescription: `
Initializes proximax configuration files and generates a new keypair.

If you are going to run ProximaX in server environment, you may want to
initialize it using 'server' profile.

For the list of available profiles see 'proximax config profile --help'
...
`,
	...
	PreRun: func(req *cmds.Request, env cmds.Environment) error {
		...
		fmt.Println("checking if daemon is running...")
		if daemonLocked {
			fmt.Println("proximax daemon is running")
			e := "proximax daemon is running. please stop it to run this command"
...
var errRepoExists = errors.New(`proximax configuration file already exists!
...
func doInit(out io.Writer, repoRoot string, empty bool, nBitsForKeypair int, confProfiles []string, conf *config.Config) error {
	if _, err := fmt.Fprintf(out, "initializing ProximaX node at %s\n", repoRoot); err != nil {
		return err
	}
...
func addDefaultAssets(out io.Writer, repoRoot string) error {
	...
	fmt.Println("init: seeded init docs %s", dkey)

	if _, err = fmt.Fprintf(out, "to get started, enter:\n"); err != nil {
		return err
	}

	_, err = fmt.Fprintf(out, "\n\tipfs cat /proximax/%s/readme\n\n", dkey)
	return err
}
