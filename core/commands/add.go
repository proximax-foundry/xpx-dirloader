...
import (
	...
	"github.com/ipfs/go-xpx-loader/params"
...
const (
	...
	hashOptionName        = "hash"
	uploadPathFile        = "uploadparam"
)

const adderOutChanSize = 8

var ProximaxUploadCmd = &cmds.Command{
...
`,
	...
	Options: []cmdkit.Option{
		...
		cmdkit.StringOption(uploadPathFile, "Json File that specifies the upload path"),
	},
	...
	PostRun: cmds.PostRunMap{
		cmds.CLI: func(req *cmds.Request, re cmds.ResponseEmitter) cmds.ResponseEmitter {
			...
			LOOP:
				...
				//	Read the file and marshal
				uploadPathFileData, _ := ioutil.ReadFile(req.Options[uploadPathFile].(string))

				//	unmarshal
				var uploadPathJson = &params.Upload{}
				json.Unmarshal(uploadPathFileData, &uploadPathJson)

				fmt.Println(uploadPathJson.PrivateKey)

				//	Do something with the output.Hash
				fmt.Fprintf(os.Stdout, "Last Hash --: %s\n", lastHash)
				fmt.Fprintf(os.Stdout, "Last File --: %s\n", lastFile)
				fmt.Fprintf(os.Stdout, "Last Bytes --: %s\n", lastBytes)
				fmt.Fprintf(os.Stdout, "Last Name --: %s\n", lastFileName)

				//	Build the ResourceHashMessage
				metaDataJson, _ := json.Marshal(uploadPathJson.Metadata)
				var resourceHashMessage = params.ResourceHashMessage{
					ContentType: "",
					Hash:        lastHash,
					Name:        lastFileName,
					MessageType: "PLAIN",
					Metadata:    string(metaDataJson),
					Keywords:    uploadPathJson.Keywords,
					PrivateKey:  uploadPathJson.PrivateKey,
					PublicKey:   uploadPathJson.PublicKey,
				}

				//	Create a NEM Txn for this content.
				hash := uploadFileOrDir(resourceHashMessage)
				fmt.Fprintf(os.Stdout, "NEM Blockchain Hash --: %s\n", hash)
			}
...
func uploadFileOrDir(resourceHashMessage params.ResourceHashMessage) string {
	fmt.Println("Loading Directory....")

	jsonValue, _ := json.Marshal(resourceHashMessage)
	response, err := http.Post("http://127.0.0.1:8891/upload/local/resource", "application/json", bytes.NewBuffer(jsonValue))
