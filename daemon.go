...
const (
	...
	javaWsAPIProfile          = "wsprofile"
	javaWsAPIConfig           = "wsconfig"
	javaWsAPIJar              = "wsjar"
	// apiAddrKwd    = "address-api"
	// swarmAddrKwd  = "address-swarm"
)

var proximaxDaemonCmd = &cmds.Command{
	Helptext: cmdkit.HelpText{
		Tagline: "Run a network-connected ProximaX node.",
		ShortDescription: `
'proximax daemon' runs a persistent proximax daemon that can serve commands
		Tagline: "Run a network-connected ProximaX node.",
		Tagline: "Run a network-connected ProximaX node.",
over the network. Most applications that use ProximaX will do so by
communicating with a daemon over the HTTP API. While the daemon is
running, calls to 'proximax' commands will be sent over the network to
the daemon.
`,
		LongDescription: `
The daemon will start listening on ports on the network, which are
documented in (and can be modified through) 'proximax config Addresses'.
For example, to change the 'Gateway' port:

  proximax config Addresses.Gateway /ip4/127.0.0.1/tcp/8082

The API address can be changed the same way:

  proximax config Addresses.API /ip4/127.0.0.1/tcp/5002
...
other computers in the network, use 0.0.0.0 as the ip address:
  ...
  proximax config Addresses.Gateway /ip4/0.0.0.0/tcp/8080
...
proximax supports passing arbitrary headers to the API and Gateway. You can
do this by setting headers on the API.HTTPHeaders and Gateway.HTTPHeaders
keys:

  proximax config --json API.HTTPHeaders.X-Special-Header '["so special :)"]'
  proximax config --json Gateway.HTTPHeaders.X-Special-Header '["so special :)"]'
...
You can setup CORS headers the same way:
  ...
  proximax config --json API.HTTPHeaders.Access-Control-Allow-Origin '["example.com"]'
  proximax config --json API.HTTPHeaders.Access-Control-Allow-Methods '["PUT", "GET", "POST"]'
  proximax config --json API.HTTPHeaders.Access-Control-Allow-Credentials '["true"]'
...
proximax uses a repository in the local file system. By default, the repo is
...
`,
	...
	Options: []cmdkit.Option{
		cmdkit.BoolOption(initOptionKwd, "Initialize proximax with default settings if not already initialized"),
		cmdkit.StringOption(initProfileOptionKwd, "Configuration profiles to apply for --init. See proximax init --help for more"),
		cmdkit.StringOption(routingOptionKwd, "Overrides the routing option").WithDefault("default"),
		cmdkit.BoolOption(mountKwd, "Mounts ProximaX to the filesystem"),
		cmdkit.BoolOption(writableKwd, "Enable writing objects (with POST, PUT and DELETE)"),
		cmdkit.StringOption(ipfsMountKwd, "Path to the mountpoint for XPXFS (if using --mount). Defaults to config setting."),
		cmdkit.StringOption(ipnsMountKwd, "Path to the mountpoint for XPXNS (if using --mount). Defaults to config setting."),
		...
		cmdkit.BoolOption(enableFloodSubKwd, "Instantiate the proximax daemon with the experimental pubsub feature enabled."),
		...
		cmdkit.StringOption(javaWsAPIProfile, "ProximaX Java WS Profile").WithDefault("local"),
		cmdkit.StringOption(javaWsAPIJar, "ProximaX Java WS Jar").WithDefault("xpx-dirloader-ws.jar"),
		cmdkit.StringOption(javaWsAPIConfig, "ProximaX Java WS Jar config.properties file").WithDefault("config.properties"),
...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) {
	...
	err := mprome.Inject()
	if err != nil {
		fmt.Println("Injecting prometheus handler for metrics failed with message: %s\n", err.Error())
	}
	...
	if managefd {
		if err := utilmain.ManageFdLimit(); err != nil {
			fmt.Println("setting file descriptor limit: %s", err)
		}
	}
	...
	if unencrypted {
		fmt.Println(`Running with --%s: All connections are UNENCRYPTED.
	...
	//	Call ProximaX Java WS
	runProximaXJavaWSAPI(req)
...
func printSwarmAddrs(node *core.IpfsNode) {
	...
	if err != nil {
		fmt.Println("failed to read listening addresses: %s", err)
	}
...
func runProximaXJavaWSAPI(req *cmds.Request) {
	fmt.Println("ProximaX Signer is Ready: " + req.Options[javaWsAPIProfile].(string) + " " + req.Options[javaWsAPIConfig].(string) + " " + req.Options[javaWsAPIJar].(string))
	_, err := exec.Command("java",
		"-Dspring.profiles.active="+req.Options[javaWsAPIProfile].(string),
		"-DconfigPath="+req.Options[javaWsAPIConfig].(string),
		"-jar", req.Options[javaWsAPIJar].(string),
	).Output()
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("Running ProximaX Signer - End (Output)")
}
