...
import (
	...
	xpxcmds "github.com/ipfs/go-xpx-loader/core/commands"
)
...
// Commands in loccommandsClientCmdalCommands should always be run locally (even if daemon is running).
// They can override subcommands in commands.Root by defining a subcommand with the same name.
var localCommands = map[string]*cmds.Command{
	"daemon":   proximaxDaemonCmd,
	"init":     proximaxInitCmd,
...
func init() {
	// set the localCommands and append to root.
	//
...
//	This is to override the custom commands.
func overrideCustomCommands() {
	localCommands["cat"] = xpxcmds.ProximaxDownloadCmd
	localCommands["add"] = xpxcmds.ProximaxUploadCmd
}
